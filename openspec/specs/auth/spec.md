...
`{orders, billing, support, reports}`; `finance` → `{billing, reports}`;
`admin` → the wildcard `ALL_TAGS` (`"*"`) rather than an explicit list, so
admin stays a true superset that covers later-composed domains without the
map being edited.
